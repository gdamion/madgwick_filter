...
{
	char *line = NULL;
	int fd = open("data/data.csv", O_RDONLY);
	_Bool start = 1;
	char **col;
	...
	while (get_next_line(fd, &line))
	{
		...
		filterUpdate(strtof(col[4], NULL), strtof(col[5], NULL), strtof(col[6], NULL),\
					strtof(col[1], NULL), strtof(col[2], NULL), strtof(col[3], NULL),\
					strtof(col[7], NULL), strtof(col[8], NULL), strtof(col[9], NULL));
       write_to_files();
       int i = 0;
       while(col[i])
        {
